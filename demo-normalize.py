...
'''
####################################################################
# Demo for normalization
# Yuchen Jin @ cainmagi@gmail.com
# Requirements: (Pay attention to version)
#   python 3.6
#   tensorflow r1.13+
#   numpy, matplotlib
# Test different kinds of normalization layers. The performance
# seems to be reduced by introducing normalization in this example.
# Use
# ```
# python demo-normalize.py -m tr -s inst -mm inst
# ```
# to train the network. Then use
# ```
# python demo-normalize.py -m ts -s inst -mm inst -rd model-...
# ```
...
def build_model(mode='split'):
    ...
    conv_1 = tf.keras.layers.Conv2D(channel_1, (3, 3), use_bias=use_bias, activation=None, padding='same')(input_img)
    if mode.casefold() == 'batch':
        conv_1 = tf.keras.layers.BatchNormalization()(conv_1)
    elif mode.casefold() == 'inst':
        conv_1 = mdnt.layers.normalize.InstanceNormalization(axis=-1)(conv_1)
    elif mode.casefold() == 'group':
        conv_1 = mdnt.layers.normalize.GroupNormalization(axis=-1, groups=8)(conv_1)
    conv_1 = tf.keras.layers.PReLU(shared_axes=[1,2])(conv_1)
    conv_2 = tf.keras.layers.Conv2D(channel_2, (3, 3), use_bias=use_bias, activation=None, padding='same')(conv_1)
    if mode.casefold() == 'batch':
        conv_2 = tf.keras.layers.BatchNormalization()(conv_2)
    elif mode.casefold() == 'inst':
        conv_2 = mdnt.layers.normalize.InstanceNormalization(axis=-1)(conv_2)
    elif mode.casefold() == 'group':
        conv_2 = mdnt.layers.normalize.GroupNormalization(axis=-1, groups=8)(conv_2)
    conv_2 = tf.keras.layers.PReLU(shared_axes=[1,2])(conv_2)
    conv_3 = tf.keras.layers.Conv2D(1, (3, 3), use_bias=use_bias, activation=None, padding='same')(conv_2)
    if mode.casefold() == 'batch':
        conv_3 = tf.keras.layers.BatchNormalization()(conv_3)
    elif mode.casefold() == 'inst':
        conv_3 = mdnt.layers.normalize.InstanceNormalization(axis=-1)(conv_3)
    elif mode.casefold() == 'group':
        conv_3 = mdnt.layers.normalize.GroupNormalization(axis=-1, groups=1)(conv_3)
    conv_3 = tf.keras.layers.Activation('sigmoid')(conv_3)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-mm', '--modelMode', default='none', metavar='str',
        help='''\
        The mode of this demo.
            none:  do not use normalization.
