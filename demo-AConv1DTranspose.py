...
#   tensorflow r1.13+
#   numpy, matplotlib
# Test the performance of modern 1D transposed convolutional layers.
# Use
# ```
# python demo-normalize.py -m tr -s tc1inst -mm inst
# ```
# to train the network. Then use
# ```
# python demo-normalize.py -m ts -s tc1inst -mm inst -rd model-...
# ```
...
def build_model(mode='bias'):
    ...
    conv_1 = mdnt.layers.AConv1D(channel_1, 9, strides=4, normalization=mode.casefold(), activation='prelu', padding='same')(input_img)
    conv_2 = mdnt.layers.AConv1D(channel_2, 5, strides=4, normalization=mode.casefold(), activation='prelu', padding='same')(conv_1)
    deconv_1 = mdnt.layers.AConv1DTranspose(channel_1, 5, strides=4, normalization=mode.casefold(), activation='prelu', padding='same')(conv_2)
    deconv_2 = mdnt.layers.AConv1DTranspose(1, 9, strides=4, normalization='bias', activation=tf.nn.sigmoid, padding='same')(deconv_1)
